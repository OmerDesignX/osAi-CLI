...
struct lora_merge_ctx {
    ...
    lora_merge_ctx(
            ...
            std::string & outfile,
            int n_threads,
            bool keep_quantized) : base_model(base_fname, 0), n_threads(n_threads), keep_quantized(keep_quantized), fout(outfile, std::ios::binary) {
    ...
    void merge_tensor(struct ggml_tensor * base, struct ggml_tensor * out) {
        ...
        if (keep_quantized && ggml_get_type_traits_cpu(out->type)->from_float == nullptr) {
            throw std::runtime_error(
                "cannot directly requantize tensor " + name_base + " to " + ggml_type_name(out->type));
        }
...
static void print_usage(int, char ** argv) {
    ...
    printf("  %s -m base-model.gguf --lora lora-file.gguf -o merged-model.gguf --keep-quantized\n", argv[0]);
    printf("\nNOTE: default output changes adapted tensors to F16\n");
    printf("NOTE: --keep-quantized preserves every base tensor type\n");
    printf("\n");
}
...
int main(int argc, char ** argv) {
    ...
    bool keep_quantized = false;
    std::vector<char *> filtered_argv;
    filtered_argv.reserve(argc);
    for (int i = 0; i < argc; ++i) {
        if (strcmp(argv[i], "--keep-quantized") == 0) {
            keep_quantized = true;
        } else {
    ...
    params.out_file = keep_quantized ? "ggml-lora-merged-quantized.gguf" : "ggml-lora-merged-f16.gguf";
    ...
    try {
        lora_merge_ctx ctx(params.model.path, params.lora_adapters, params.out_file, params.cpuparams.n_threads, keep_quantized);
        ctx.run_merge();
