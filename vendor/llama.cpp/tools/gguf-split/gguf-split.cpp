...
static void gguf_merge(const split_params & split_params) {
    ...
    for (int i_split = 0; i_split < n_split; i_split++) {
        ...
        if (i_split == 0) {
            ...
            // Exporters reject a merged file if split keys remain.
            gguf_remove_key(ctx_gguf, LLM_KV_SPLIT_NO);
            gguf_remove_key(ctx_gguf, LLM_KV_SPLIT_COUNT);
            gguf_remove_key(ctx_gguf, LLM_KV_SPLIT_TENSORS_COUNT);
