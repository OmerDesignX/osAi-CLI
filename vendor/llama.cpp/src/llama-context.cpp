...
void llama_context::opt_init(struct llama_model * model, struct llama_opt_params lopt_params) {
    ...
    opt_params.opt_period      = lopt_params.opt_period > 0
            ? lopt_params.opt_period
            : n_batch / n_ubatch;
    opt_period = opt_params.opt_period;
  ...
  //llama_set_param(model->tok_embd,        param_filter, param_filter_ud); // FIXME
    ...
    // Active LoRA tensors are regular graph inputs, but they are owned by the
    // adapter rather than llama_model. Register them with the optimizer so a
    // filter can select adapter-only training while every quantized base tensor
    // remains frozen in its original packed representation.
...
void llama_context::opt_epoch_iter(
    ...
    while (n_ctx_active > 0 && labels_sparse[n_ctx_active - 1] < 0) {
        --n_ctx_active;
    }

    memory->clear(true);

    for (uint32_t pos_ctx = 0; pos_ctx < n_ctx_active; pos_ctx += n_batch) {
        ...
        do {
            ...
            uint32_t n_labels = 0;
            bool has_future_labels = false;
            for (uint32_t pos_ubatch = 0; pos_ubatch < ubatch.n_tokens; ++pos_ubatch) {
                const uint32_t ilabel = pos_ctx + pos_batch + pos_ubatch;
                n_labels += labels_sparse[ilabel] >= 0;
            }
            for (uint32_t ilabel = pos_ctx + pos_batch + ubatch.n_tokens;
                    ilabel < n_ctx_active; ++ilabel) {
                if (labels_sparse[ilabel] >= 0) {
                    has_future_labels = true;
                    break;
                }
            }
            // Assistant-only examples are padded to n_ctx. Once their last
            // supervised token has been consumed, avoid applying optimizer
            // momentum or weight decay to padding-only microbatches.
            if (n_labels == 0 && !has_future_labels) {
                break;
            }
            const bool train_ubatch = train && n_labels > 0;
            ...
            ggml_opt_alloc(opt_ctx, train_ubatch);
            ...
            res->set_inputs(&ubatch);
            {
                ...
                // Cross entropy divides by every row. Weight the active rows
                // so masked prompt positions do not dilute assistant loss.
                const bool weighted = example_label_count > 0;
                const float label_weight = weighted
                        ? (float) n_ubatch * example_weight * opt_period / example_label_count
                        : (n_labels > 0 ? (float) n_ubatch/n_labels : 0.0f);
                for (uint32_t pos_ubatch = 0; pos_ubatch < n_ubatch; ++pos_ubatch) {
                    ...
                    ggml_backend_tensor_set(labels, &label_weight, (pos_ubatch*labels->ne[0] + labels_sparse[ilabel])*sizeof(float), sizeof(float));
                }
            }
            if (train_ubatch) {
                std::fprintf(stderr, "supervised optimizer step labels=%u\n", n_labels);
            }
            // Prompt-only chunks are needed to populate causal state, but they
            // are not datapoints in an assistant-only loss. Excluding them
            // keeps sequence-normalized evaluation independent of prompt size.
            ggml_opt_eval(opt_ctx, n_labels > 0 ? result : nullptr);
            if (callback) {
...
void llama_context::opt_epoch(
    ...
    opt_epoch_weighted(dataset, result_train, result_eval, idata_split,
