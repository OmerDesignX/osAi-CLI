...
int32_t llama_adapter_lora_save_to_file(const llama_adapter_lora * adapter, const char * path_lora) {
    if (adapter == nullptr || adapter->model == nullptr || path_lora == nullptr || path_lora[0] == '\0') {
        LLAMA_LOG_ERROR("%s: invalid adapter or output path\n", __func__);
        return 1;
    }
    ...
    try {
        ...
        const std::string architecture = adapter->model->arch_name();
        const auto name_it = adapter->gguf_kv.find("general.name");
        const std::string name = name_it == adapter->gguf_kv.end()
                ? "llama.cpp trained LoRA"
                : name_it->second;

        gguf_set_val_str(ctx_gguf.get(), "general.architecture", architecture.c_str());
        gguf_set_val_str(ctx_gguf.get(), "general.name", name.c_str());
        ...
        if (!adapter->alora_invocation_tokens.empty()) {
            gguf_set_arr_data(
                    ctx_gguf.get(),
                    "adapter.alora.invocation_tokens",
                    GGUF_TYPE_UINT32,
                    adapter->alora_invocation_tokens.data(),
                    adapter->alora_invocation_tokens.size());
        }
        ...
        for (const auto & [name_base, weight] : weights) {
            GGML_UNUSED(name_base);
            if (weight->a == nullptr || weight->b == nullptr) {
                throw std::runtime_error("adapter contains an incomplete LoRA tensor pair");
            }
            gguf_add_tensor(ctx_gguf.get(), weight->a);
            gguf_add_tensor(ctx_gguf.get(), weight->b);
        }

        if (!gguf_write_to_file(ctx_gguf.get(), path_lora, false)) {
            throw std::runtime_error("failed to write GGUF file");
        }
        LLAMA_LOG_INFO("%s: saved %zu tensors to '%s'\n", __func__, weights.size()*2, path_lora);
        return 0;
    } catch (const std::exception & err) {
        LLAMA_LOG_ERROR("%s: failed to save lora adapter: %s\n", __func__, err.what());
        return 2;
    }
}
