...
void ggml_backend_sched_split_graph(ggml_backend_sched_t sched, struct ggml_cgraph * graph) {
    ...
    for (int i = 0; i < graph->n_nodes; i++) {
        ...
        if (*cur_backend_id == -1) {
            ...
            GGML_ABORT(
                    "no backend supports tensor '%s' with operation %s; src0=%s [%" PRId64 ",%" PRId64 ",%" PRId64 ",%" PRId64 "]; src1=%s [%" PRId64 ",%" PRId64 ",%" PRId64 ",%" PRId64 "]",
                    node->name,
                    ggml_op_desc(node),
                    src0 ? ggml_type_name(src0->type) : "none",
                    src0 ? src0->ne[0] : 0, src0 ? src0->ne[1] : 0,
                    src0 ? src0->ne[2] : 0, src0 ? src0->ne[3] : 0,
                    src1 ? ggml_type_name(src1->type) : "none",
                    src1 ? src1->ne[0] : 0, src1 ? src1->ne[1] : 0,
                    src1 ? src1->ne[2] : 0, src1 ? src1->ne[3] : 0);
        }
    }
