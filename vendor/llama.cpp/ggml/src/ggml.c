            ...
            switch (ggml_get_unary_op(tensor)) {
                ...
                case GGML_UNARY_OP_SIGMOID: {
                    if (src0_needs_grads) {
                        ...
                        // Reuse the forward result to avoid evaluating sigmoid twice.
                        struct ggml_tensor * derivative = ggml_sub(
                                ctx, tensor, ggml_mul(ctx, tensor, tensor));
                        ggml_add_or_set(ctx, cgraph, isrc0, ggml_mul(ctx, derivative, grad));
                    }
                } break;
