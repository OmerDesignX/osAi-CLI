...
int ggml_metal_device_count(void) {
    @autoreleasepool {
        ...
        const int count = (int) devices.count;
        [devices release];
        return count;
#else
        id<MTLDevice> device = MTLCreateSystemDefaultDevice();
        const int count = device == nil ? 0 : 1;
        [device release];
        return count;
#endif
    }
...
ggml_metal_device_t ggml_metal_device_init(int device, int n_devices, int device_phys, int device_virt) {
    ...
    @autoreleasepool {
        if (dev->mtl_device == nil) {
#if TARGET_OS_OSX
            NSArray<id<MTLDevice>> * devices = MTLCopyAllDevices();
            if (device_phys >= 0 && device_phys < (int) devices.count) {
                dev->mtl_device = [[devices objectAtIndex:device_phys] retain];
            }
            [devices release];
#else
            if (device_phys == 0) {
            ...
            if (dev->mtl_device) {
                ...
                dev->props.device_phys = device_phys;
                dev->props.device_virt = device_virt;
...
bool ggml_metal_buffer_cpy_tensor(ggml_metal_buffer_t buf_dst, const struct ggml_tensor * src, struct ggml_tensor * dst) {
    ...
    if (buf_dst->dev->mtl_device != buf_src->dev->mtl_device) {
        return false;
    }
