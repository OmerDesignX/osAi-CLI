...
static int g_physical_devices = 0;
...
static ggml_backend_dev_t ggml_backend_metal_device_init(ggml_backend_reg_t reg, int device, int device_phys, int device_virt) {
    return new ggml_backend_device {
        /* .iface   = */ ggml_backend_metal_device_i,
        /* .reg     = */ reg,
        /* .context = */ ggml_metal_device_get(device, g_devices, device_phys, device_virt),
    };
}
...
ggml_backend_reg_t ggml_backend_metal_reg(void) {
...

    {
        ...
        if (!initialized) {
            ...
            g_physical_devices = ggml_metal_device_count();
            g_devices = g_physical_devices;
            const char * env = getenv("GGML_METAL_DEVICES");
            if (env && g_physical_devices > 0) {
                const int requested = atoi(env);
                if (requested > 0) {
                    g_devices = std::min(requested, GGML_METAL_MAX_DEVICES);
                }
            }

            for (int i = 0; i < g_devices; ++i) {
                const int device_phys = i % g_physical_devices;
                const int device_virt = i / g_physical_devices;
                auto * dev = ggml_backend_metal_device_init(&reg, i, device_phys, device_virt);
