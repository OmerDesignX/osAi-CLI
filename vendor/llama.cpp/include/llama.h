...
extern "C" {
    ...
    // Save the adapter's current tensors to a GGUF file.
...
#define LLAMA_STATE_SEQ_FLAGS_ON_DEVICE 2
    ...
    struct llama_opt_params {
        ...
        int32_t opt_period; // override gradient accumulation period; 0 derives it from batch sizes
    };
    ...
    // Like llama_opt_epoch, but scale each datapoint's normalized sparse-label
    // cross entropy before accumulating gradients. This permits exact
    // sequence-level preference gradients without making the packed base
    // trainable. label_counts contains the active token count per datapoint.
