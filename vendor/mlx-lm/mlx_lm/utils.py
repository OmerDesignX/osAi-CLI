...
def _osai_embedded_adapter(model_path: Path) -> Optional[str]:
    """Resolve an adapter embedded by osai without leaving the model directory."""

    root = Path(model_path).resolve()
    manifest = root / "osai_fusion.json"
    if not manifest.is_file():
        return None
    try:
        payload = json.loads(manifest.read_text(encoding="utf-8"))
    except (OSError, UnicodeError, json.JSONDecodeError) as exc:
        raise ValueError(f"Invalid osai fusion manifest {manifest}: {exc}") from exc
    if (
        payload.get("schema_version") != 1
        or payload.get("kind") != "osai-mlx-quantized-residual-v1"
    ):
        raise ValueError(f"Unsupported osai fusion manifest: {manifest}")
    value = payload.get("adapter_path")
    if not isinstance(value, str) or not value:
        raise ValueError(f"Missing adapter_path in osai fusion manifest: {manifest}")
    relative = Path(value)
    if relative.is_absolute() or ".." in relative.parts:
        raise ValueError(f"Unsafe adapter_path in osai fusion manifest: {manifest}")
    adapter = (root / relative).resolve()
    try:
        adapter.relative_to(root)
    except ValueError as exc:
        raise ValueError(f"Embedded adapter escapes model directory: {manifest}") from exc
    required = (adapter / "adapter_config.json", adapter / "adapters.safetensors")
    if any(not path.is_file() or path.stat().st_size == 0 for path in required):
        raise FileNotFoundError(f"Incomplete embedded adapter in {adapter}")
    return str(adapter)
...
def load(
    ...
    Args:
        ...
        model_config(dict, optional): Configuration parameters specifically for the model.
            Defaults to an empty dictionary.
        adapter_path (str, optional): Path to the LoRA adapters. If omitted, an adapter embedded
            by osai is applied automatically. Default: ``None``.
        lazy (bool): If ``False`` eval the model parameters to make sure they are
            loaded in memory before returning, otherwise they will be loaded
            when needed. Default: ``False``
    ...
    if adapter_path is None:
        adapter_path = _osai_embedded_adapter(model_path)
